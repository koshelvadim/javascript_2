...
class List {
  constructor(url, container, list = listContext){
    this.container = container; // блок отрисовки каталога или корзины или чего либо
    this.list = list; // словарь для классов строка 213
    this.url = url;  // ссылка
    this.goods = [];  // массив для хранения товаров, полученных с сервера
    this.allProducts = [];  //  массив для хранения товаров, положенных в корзину
    this.filtered = []; // массив для отфильтрованных товаров
    this._init();  // вызываем метод 
   ...
   */
  getJson(url){
    return fetch(url ? url : `${API + this.url}`)  //проверка есть ли URL
   ...
   */
  handleData(data){
    this.goods = data;  // хранение данных от сервера 
  ...
  /**
   * метод добавления блока в разметку (каталог или корзина или другое)
   * @returns 
   */
  render(){
 ...
 */
class ProductsList extends List{  // класс каталога
  ...
  // обработчики событий каталога (кнопка Купить и Поиск)
  _init(){
...
// класс элемента каталога с методом разметки одного элемента
...
class Cart extends List{  // класс корзины
  ...
  // обработчики событий корзины (кнока корзина и удаления товара из корзины )
  _init(){  
